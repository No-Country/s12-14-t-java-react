...
export const useEmployee = () => {
  ...
  const deleteOneEmployee = async (id) => {
    pageApi
      .delete(
        `${import.meta.env.VITE_API_URL}/employee/deleteEmployee/${id}`,
        config
      )
      .then((response) => {
  ...
  const deleteSequentially = async (lista, listaEmpleadosCopia) => {
    ...
    while (lista.length > 0) {
      ...
      try {
        ...
        errores.push(error);
        console.log(
          "Error en eliminar secuencialmente con el id:",
          idAEliminar,
          "error encontrado: ",
          error
        );
  ...
  const deleteEmployees = async (listaEmpleados, listaEmpleadosCopia) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        deleteSequentially(listaEmpleados, listaEmpleadosCopia).catch(
          (error) => {
